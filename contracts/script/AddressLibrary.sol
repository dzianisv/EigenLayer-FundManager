...
library AddressLibrary {
    ...
    function toAddress(string memory str) public pure returns (address addr) {
        ...
        require(strBytes.length == 42, "Invalid input, should be 42 characters long including '0x'.");
        require(strBytes[0] == '0' && strBytes[1] == 'x', "Address string should start with '0x'.");

        uint160 total = 0;
        uint160 base = 1;

        // Start from the end of the string and ignore the '0x' prefix
        for (uint256 i = strBytes.length - 1; i >= 2; i--) {
